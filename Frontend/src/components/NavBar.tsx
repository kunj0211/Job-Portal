...
const NavBar = () => {
	...
	return (
		<div className='sticky top-0 z-50 w-full h-18 bg-transparent'>
			<nav
							...
							{navLinks.map((link) => (
								...
								>
									{({ isActive }) => (
										<>
											<link.icon
												className={`text-lg transition-colors ${isActive ? 'text-emerald-600' : 'text-slate-400 group-hover:text-slate-600'}`}
											/>
											{link.name}
											{isActive && (
												<motion.div
