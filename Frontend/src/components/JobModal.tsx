...
const JobModal = ({ isOpen, onClose, jobToEdit }: JobModalProps) => {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<JobFormValues>({
  ...
  const onSubmit = async (data: JobFormValues) => {
    try {
      if (jobToEdit && jobToEdit.id) {
        await dispatch(updateJob({ id: jobToEdit.id, jobData: data })).unwrap();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto">
          <form id="jobForm" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                />
                {errors.title && <p className="mt-1 text-xs text-red-500 text-left">{errors.title.message}</p>}
              </div>
              ...
              <div>
                ...
                />
                {errors.company && <p className="mt-1 text-xs text-red-500 text-left">{errors.company.message}</p>}
              </div>
              ...
              <div>
                ...
                />
                {errors.location && <p className="mt-1 text-xs text-red-500 text-left">{errors.location.message}</p>}
              </div>
              ...
              <div>
                ...
                {errors.jobType && <p className="mt-1 text-xs text-red-500 text-left">{errors.jobType.message}</p>}
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Salary Range (Optional)</label>
                <input 
                  {...register('salaryRange')}
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
                  placeholder="2 lakh - 4 lakh"
                />
                {errors.salaryRange && <p className="mt-1 text-xs text-red-500 text-left">{errors.salaryRange.message}</p>}
              </div>
              ...
              <div>
                ...
                />
                {errors.experience && <p className="mt-1 text-xs text-red-500 text-left">{errors.experience.message}</p>}
              </div>
            </div>
            ...
            <div>
              ...
              ></textarea>
              {errors.description && <p className="mt-1 text-xs text-red-500 text-left">{errors.description.message}</p>}
            </div>
          </form>
        </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50/50">
          ...
          >
            {isSubmitting ? 'Saving...' : (jobToEdit ? 'Save Changes' : 'Post Job')}
          </button>
        </div>
